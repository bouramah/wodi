                    ...
                    <form method="POST" action="{{ route('deposits.update', $deposit) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="depositor_id" name="depositor_id" required
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client->id }}" @selected(old('depositor_id', $deposit->depositor_id) == $client->id)>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="amount" class="block mt-1 w-full" type="number" step="0.01"
                                min="{{ $deposit->amount - $deposit->remaining_amount }}"
                                name="amount" :value="old('amount', $deposit->amount)" required />
                            <p class="text-sm text-gray-500 mt-1">
                                {{ __('Le montant ne peut pas être inférieur au montant déjà utilisé:') }}
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="deposit_date" class="block mt-1 w-full" type="date"
                                name="deposit_date" :value="old('deposit_date', $deposit->deposit_date->format('Y-m-d'))" required />
                            @error('deposit_date')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
